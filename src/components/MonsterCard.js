...
function MonsterCard({ monsterID, monsterOldName, monsterFamily }) {
  ...
  return (
    <Box className="w-full h-full">
      <Card
        asChild
        className="group hover:shadow-md hover:shadow-black/20 transition-shadow duration-300 hover:outline hover:outline-2 hover:outline-blue-500"
      >
        <Link key={monsterID} to={`${monsterID}`}>
          ...
          <Text
            ...
            weight={"bold"}
            color="blue"
            className="group-hover:underline text-sm sm:text-base"
          >
            {monsterOldName}
          </Text>
          <Text
